...
//     prerequisite ended `Executed`. A failed/skipped prerequisite makes this
//     leg `SkippedUnmetDependency` and `place_leg` is NOT called — the skip then
//     propagates transitively to its own dependents.
...
//   Failed                — `place_leg` returned an Error.
//   SkippedUnmetDependency— a prerequisite was Failed/Skipped, so this leg was
//                           NEVER sent (the never-orphan invariant).
//   Unwound               — was Executed, then cancelled/exited during
//                           remediation under `UnwindExecuted`.
enum class LegStatus { Executed, Failed, SkippedUnmetDependency, Unwound };
...
//   PartiallyExecutedUnwound — >=1 leg failed/skipped; executed legs were
//                              unwound per `UnwindExecuted` (Critical alert).
//   PartiallyExecutedLeft    — >=1 leg failed/skipped; executed legs LEFT in
//                              place per `LeaveAndAlert` (Warning alert).
...
enum class BasketOutcome { Complete, PartiallyExecutedUnwound, PartiallyExecutedLeft, Blocked };
...
//      and the skip propagates to dependents. `place_leg` Error => `Failed`.
//   3. PARTIAL + POLICY — if any leg is Failed/Skipped: `UnwindExecuted` unwinds
//      executed legs newest-first (null/Error unwind leaves the leg Executed +
//      Critical detail) then sends ONE Critical alert =>
